...
if ($countResult) {
    ...
    echo "Total records found before year 2000: " . number_format($totalRecords) . "\n";
} else {
    die("Error getting count: " . $con->error . "\n");
}

// Open file for writing
$outputFile = '/files_to_delete.txt';
...
// Query to get pgpub_doc_num and pgpub_date for records before year 2000
...
$batchSize = 10000;
$basePath = '/mnt/volume_sfo2_12/applications/XML/';

// Process results and write to file
while ($row = $result->fetch_assoc()) {
    $pgpubDocNum = $row['pgpub_doc_num'];
    $pgpubDate = $row['formatted_date'];
    
    // Format: US{pgpub_doc_num}A1-{pgpub_date}.XML
    $filename = "US" . $pgpubDocNum . "A1-" . $pgpubDate . ".XML";
