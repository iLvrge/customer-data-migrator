			...
			if($orgConnect) {
				...
				if(count($allRepresentative) > 0 ) { 
					...
					$queryFindAllRFIDs .= " ) GROUP BY appno_doc_num ) AND (
						...
						OR 
						MATCH(a.convey_text) AGAINST('\"CONTINUATION\"' IN BOOLEAN MODE)
						OR 
				...
				if(count($allRepresentative) > 0 ) {
				   ...
				   $con->query($queryUpdate);*/
					...
					$queryFindAllRFIDs .= " ) GROUP BY appno_doc_num )  AND MATCH(a.convey_text) AGAINST('\"CHANGE OF NAME\"' IN BOOLEAN MODE)  GROUP BY documentid.rf_id";
				...
				if(count($allRepresentative) > 0  ) {
					$queryFindAllRFIDs = "SELECT d.rf_id FROM documentid AS d INNER JOIN assignment as a ON a.rf_id = d.rf_id  INNER JOIN assignment_conveyance as rac ON rac.rf_id = a.rf_id INNER JOIN db_uspto.list2 as l2 ON l2.rf_id = d.rf_id WHERE rac.convey_ty IN('missing', 'namechg', 'other') AND ( l2.organisation_id = 0  OR l2.organisation_id IS NULL ) ";
					
					if( count($allRepresentative) > 0 ) {
						$queryFindAllRFIDs .= " AND l2.company_id IN (".implode(',', $allRepresentative).") ";
					}
					/* $queryFindAllRFIDs .= "  ) GROUP BY appno_doc_num ) AND (
		...
		MATCH(a.convey_text) AGAINST('\"SECURITY\" -RELEASE -DISCHARGE' IN BOOLEAN MODE) OR MATCH(a.convey_text) AGAINST('\"SUCCESSION OF AGENCY\" -RELEASE -DISCHARGE' IN BOOLEAN MODE) ) GROUP BY documentid.rf_id";
			...
			MATCH(a.convey_text) AGAINST('\"SECURITY\" -RELEASE -DISCHARGE' IN BOOLEAN MODE) OR MATCH(a.convey_text) AGAINST('\"SUCCESSION OF AGENCY\" -RELEASE -DISCHARGE' IN BOOLEAN MODE) ) GROUP BY documentid.rf_id";
				...
				if(count($allRepresentative) > 0) {
					...
					$queryFindAllRFIDs = "SELECT documentid.rf_id FROM documentid INNER JOIN assignment as a ON a.rf_id = documentid.rf_id  INNER JOIN assignment_conveyance as rac ON rac.rf_id = a.rf_id WHERE rac.convey_ty IN ('missing', 'security', 'other', 'govern', 'correct') AND documentid.appno_doc_num IN (SELECT appno_doc_num FROM documentid WHERE rf_id IN (
					SELECT rf_id FROM db_uspto.list2 WHERE ( organisation_id = 0  OR organisation_id IS NULL ) ";
