        ...
        if((int)$organisationID > 0) {      
            ...
            if($result && $result->num_rows > 0) {
                ...
                while($row = $result->fetch_object()) {
                    ...
                    $con->query("DELETE FROM db_new_application.log_update_company WHERE company_id = '".$row->company_id."'");

                    $logResult = $con->query("INSERT INTO db_new_application.log_update_company(message,  company_id, start_time, end_time) VALUES ('In progress',  $row->company_id, '".date('Y-m-d H:i:s')."', '".date('Y-m-d H:i:s')."')" );  
                    ...
                    echo "UPDATE db_new_application.log_update_company SET `status` = 1, `message` = 'Finished', `end_time` = '".date('Y-m-d H:i:s')."' WHERE id =  $logID";

                    $con->query("UPDATE db_new_application.log_update_company SET `status` = 1, `message` = 'Finished', `end_time` = '".date('Y-m-d H:i:s')."' WHERE id =  $logID" ); 
                }    
                ...
                exec('node /var/www/html/scripts/retrieve_cited_patents_assignees.js  0 "'.json_encode($allCompanies).'" 2');
                if($runOtherScript == '0') {
