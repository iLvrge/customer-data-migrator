...
if(count($variables) > 1) {
	...
	if((int)$company > 0) {	
        ...
        $queryRepresentative = "SELECT representative_id, representative_name FROM ".$dbUSPTO.".representative WHERE  representative_id = ".$company; 
                ...
                */
                ...
                if(count($originalList) > 0  || count($expiredAssets) > 0 || count($soldAssets) > 0 || count($applicantAssets) > 0 ) {  
                    ...
                    */
                    ...
                    $clientOwnedQuery = "SELECT application FROM db_new_application.dashboard_items WHERE representative_id = ".$companyID." AND type = 30 GROUP BY application";
                    ...
                    if(count($expiredAssets) > 0) { 
                        ...
                        if(count($remainingAssets) > 0) {
                            $queryAbandonedAssets .= " AND appno_doc_num NOT IN (".implode(',', $remainingAssets).") ";
                        }
...
echo $queryPendingApplications."<br/>";
                    ...
                    $customerAllAssets = array_merge($originalList, $expiredAssets, $soldAssets,$applicantAssets);
print_r( $customerAllAssets);

echo implode(',', $customerAllAssets);
                    ...
                    $queryLenders = "INSERT IGNORE INTO ".$dbApplication.".dashboard_items (representative_id, assignor_id, type, patent, application, rf_id, total) SELECT ".$companyID.", apt.assignor_and_assignee_id, ".$type.", d.grant_doc_num,  d.appno_doc_num, apt.rf_id, 0 FROM ".$dbUSPTO.".documentid AS d 
                            ...
                            AND d.appno_doc_num IN (".implode(',', $customerAllAssets).") AND date_format(d.appno_date, '%Y') > ".$YEAR."  AND apt.activity_id IN (5, 12) AND apt.company_id = ".$companyID."
