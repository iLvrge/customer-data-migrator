...
const getPartiesLists = async (organisationID, companyID, ownedAssets, assigneeIDs) => {
    try {
        ...
        if(partiesResult.length > 0) {
            ...
            WHERE organisation_id  ${ typeof ownedAssets != 'undefined' && ownedAssets == 5 ?  '<>'  : '=' } 0  AND ao.assignee_organization IN (:allParties)`
...
( async () => {    
    try {
        ...
        if(source_data == 1) {
            getAssigneeList = await getPartiesLists(organisationID, companyID, ownedAssets, assigneeIDs) 
