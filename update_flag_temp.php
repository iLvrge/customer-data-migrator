		...
		if($resultOrganisation && $resultOrganisation->num_rows > 0) {
			...
			if($orgConnect) {
				...
				if(count($assignorAndAssigneeIDs) > 0) {
					...
					echo $queryList1 = "SELECT ass.rf_id FROM db_uspto.assignee AS ass INNER JOIN db_uspto.assignment_conveyance AS ac ON ac.rf_id = ass.rf_id  AND  ac.convey_ty IN ('missing', 'other', 'govern', 'assignment', 'employee', 'correct', 'namechg') INNER JOIN db_uspto.list2 AS li On li.rf_id = ass.rf_id AND (organisation_id = 0 OR organisation_id IS NULL) AND company_id IN (".implode(',', $allRepresentative).") GROUP BY ass.rf_id";
					...
					$list21 = array();
					if(count($list1) > 0) {
						echo $queryList2 = "SELECT MAX(doc.appno_doc_num) AS appno_doc_num FROM documentid AS doc WHERE appno_doc_num <> '' AND rf_id IN (".implode(',', $list1).") GROUP BY appno_doc_num"; 
					...
					if(count($RFID0) > 0) {
						...
						if(count($remainingRFIDs) > 0) {
							...
							echo "node /var/www/html/script/inventor_levenshtein_test.js '".$fileName."' '".$organisationID."' '".json_encode($allRepresentative)."'";

							
							$output = shell_exec("node /var/www/html/script/inventor_levenshtein_test.js '".$fileName."' '".json_encode($allRepresentative)."'");
